...
// * Los primeros `cantidad` elementos de `elementos` son los válidos del ArrayList
...
// PRECOND.: el int i dado debe ser una posición válida del array xs dado.
    if(i < 1 || i > xs->cantidad) {  
       cerr << "Error: la posición dada es mayor a la capacidad del arraylist dado" << endl;
       exit(1);
    } 
    xs->elementos[i-1] = x; 
}
